    ...
    try {
      ...
      if (!i.ok) {
        ...
        if (a > 0 && b.set(d, { until: Date.now() + a, result: p }), !f) {
          ...
          R.warnOnce(`http:${i.status}:${n}:${new URL(t, (location == null ? void 0 : location.href) ?? "http://localhost/").origin}`, `HTTP error ${w} while fetching ${n} ${t}`);
        }
      ...
      if (!/^blob:/i.test(t) && a > 0 && b.set(d, { until: Date.now() + a, result: u }), !f) {
        let p = `${c}:${n}:${new URL(t, (location == null ? void 0 : location.href) ?? "http://localhost/").origin}`, w = c === "timeout" ? `Timeout after ${o}ms. Consider increasing timeout or using a proxy for ${t}` : c === "abort" ? `Request aborted while fetching ${n} ${t}` : `Network/CORS issue while fetching ${n} ${t}. A proxy may be required`;
        R.errorOnce(p, w);
      }
...
//# sourceMappingURL=chunk-7LKQAJ4Q.js.map
